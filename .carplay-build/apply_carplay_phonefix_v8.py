...
# 2) V8 is MediaPlayer-only.  Remove every scene-based CarPlay hook if an older
# branch reintroduced one.
if 'configurationForConnectingSceneSession' in main or 'CPTemplateApplicationScene' in main or 'LXPhoneSceneDelegate' in main:
    raise SystemExit('V8 base main.m unexpectedly contains UIScene/CarPlay scene code; refusing to build a mixed architecture.')

# 3) The old provider constructor ran before UIApplicationMain/RNN established the
# phone root.  Delete it completely.  The provider will be activated from JS only
# after the bootstrap root is attached and normal app initialization has begun.
main = re.sub(
...
# after the bootstrap root is attached.  This preserves cold CarPlay support while
# preventing CarPlay framework work from racing the iPhone window startup.
